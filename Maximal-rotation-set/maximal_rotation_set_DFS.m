function [rot_set_lower_bound,rot_set_upper_bound]=maximal_rotation_set_DFS(DX,DY,margin,maxdepth,include)
...
for i=1:size(DX,1)
    for j=1:size(DY,1)
        ...
        if include(i,j)==1
            ...
            recursion(II,JJ,x_idx,y_idx,1,maxdepth,i,j,[i],[j],DX,DY,margin); %#ok<NBRAK> %recurse with (i,j)
        end
        clc
